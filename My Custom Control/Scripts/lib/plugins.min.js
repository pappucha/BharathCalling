(function () {
    ...
    function ($) {
        ...
        $.msgBox = function (msg, options) {
            var _options = options || {},
                ...
                _template = ['<div class="_alert-wrapper _scale">', '<div class="_msgbox">', '<div class="_msgbox-body"><div class="_msgbox-header">' + _title + "</div></div>", '<div class="_msgbox-footer"></div>', "</div>", "</div>"].join(""),
                _msgbox = $(_template),
                ...
                _getMsgTemplate = function () {
                    ...
                    var icon = "" === _icon ? "" : '<span class="ico ' + _icon + '"></span>',
                        _msg_template = '<p class="' + _type + '">' + icon + "<span>" + msg + "</span></p>";
                    return $(_msg_template)
                },
                _generateButtons = function () {
                    var buttons = _getButtons(),
                        btnArr = [];
                    return $.each(buttons, function (i, b) {
                        _footer.append('<input type="button" value="' + b + '" />')
                    }), btnArr
                };
            return _body.append(_getMsgTemplate()), _form = _body.find("form"), "" === $.trim(_title) && _header.hide(), _msgbox.appendTo("body"), _generateButtons(), _alert.find("input").eq(0).focus(), _alert.width(_width), _alert.height(_height), _footer.find(":button").on("click", function () {
                var _value, _result = $(this).val(),
                    _callback = _options.onComplete || function () { };
                _form.length && (_value = _form.serializeArray(), 1 == _value.length && (_value = _value[0].value)), _callback(_result, _value) !== !1 ? _msgbox.animate({
                    opacity: "0"
                }, {
                    duration: 50,
                    specialEasing: {
                ...
                }) : (_alert.removeClass("animated shake"), setTimeout(function () {
                    _alert.addClass("animated shake")
                }, 5))
            }), _msgbox.removeClass("_scale"), {
                Popup: _msgbox,
                Options: options,
                close: function () {
                    _msgbox.remove()
            ...
            return options = options || {}, options.type = "info", $.msgBox(msg, options)
        }, $.prompt = function (msg, options) {
            var _options = options || {},
                _inputs = _options.inputs || [{
                    type: "text",
                    label: msg
                }],
                _defaultTemplate = '<form onsubmit="return false"></form>',
                _template = $(_options.template || _defaultTemplate),
                _callback = options.onComplete || function () { };
            _options.height = _options.height || 80, _options.type = "prompt", _options.buttons = ["Ok", "Cancel"];
            
            var _form = _template.is('form')?_template:_template.find("form");
            0 === _form.length && (_form = _template.wrap("form")), _form.unbind("submit").on("submit", function () {
                return !1
            }), _inputs.length && $.each(_inputs, function (i, inp) {
                var elm;
                switch (inp.type) {
                    case "textarea":
                        elm = $("<textarea/>"), elm.prop("name", inp.name || "_inp_" + i), elm.val(inp.value || ""), inp.required && elm.attr("required", inp.required);
                        break;
                    case "select":
                    case "list":
                        elm = $("<select/>"), elm.prop("name", inp.name || "_inp_" + i), "list" == inp.type && elm.prop("multiple", !0), inp.required && elm.attr("required", inp.required), inp.list instanceof Array && $(inp.list).each(function (j, l) {
                            "object" == typeof l ? $("<option/>").html(l.text || l.value).prop({
                                value: l.value || l.text
                            }).appendTo(elm) : $("<option/>").html(l).prop({
                                value: l
                            }).appendTo(elm)
                        }), elm.val(inp.value || "");
                        break;
                    case "checkboxGroup":
                        elm = $("<div/>"), inp.list instanceof Array && $(inp.list).each(function (j, l) {
                            var label = $("<label>" + (l.label || "") + "</label>");
                            $('<input type="checkbox"/>').prop({
                                name: inp.name || "_inp_" + i,
                                id: inp.name + i || "_inp_" + i,
                                value: l.value,
                                checked: l.checked || !1
                            }).prependTo(label), label.appendTo(elm)
                        });
                        break;
                    case "radioGroup":
                        elm = $("<div/>"), inp.list instanceof Array && $(inp.list).each(function (j, l) {
                        ...
                        break;
                    default:
                        elm = $("<input/>"), elm.prop("name", inp.name || "_inp_" + i), elm.prop({
                            value: inp.value || "",
                            type: inp.type || "text"
                        }), inp.required && elm.attr("required", inp.required)
                }
                elm.appendTo(_form), elm.wrap('<div class="_label">' + (inp.label || "") + "</label>")
            }), _options.template = _template;
            var prompt = $.msgBox(msg, _options);
            return prompt.Options.onComplete = function (res, val) {
                if ("Cancel" != res && _form.length) {
                    var _validationElements = _form.find("[required]");
                    return _form.find(".error").removeClass("error"), _validationElements.each(function (i, el) {
                        el = $(el), "" === $.trim(el.val()) && el.addClass("error")
                    }), _form.find(".error").length ? !1 : _callback(res, val)
                }
                _callback(res, val)
            }, prompt
        }, $.confirm = function (msg, options) {
            var _options = options || {};
            return _options.type = "confirm", _options.buttons = ["Yes", "No", "Cancel"], $.msgBox(msg, _options)
        }
    }(jQuery, window),
    function ($, window) {
        "use strict";
        window.glams || (window.glams = {}), window.glams.modal || (window.glams.modal = []), window.glams.modal = {
            _popup: [],
            getAll: function () {
            ...
            getByName: function (name) {
                var _m = [];
                return $.each(window.glams.modal._popup, function (i, p) {
                    p.name == name && _m.push(p)
                }), _m
            },
            ...
            remove: function (modal) {
                ...
                return index >= 0 ? (window.glams.modal._popup.splice(index, 1), !0) : !1
            }
        }, $.modal = function (title, content, options) {
            function loadContent() {
                if (_body.html(""), _modal.removeClass("iframe"), "iframe" == _options.type) {
                    var iframe = $('<iframe frameborder="0" allowfullscreen webkitallowfullscreen mozallowfullscreen oallowfullscreen msallowfullscreen/>'),
                        loader = $('<div class="gif-loader" />');
                    loader.appendTo(_body), iframe.attr("src", content), _body.append(iframe), _modal.addClass("iframe"), iframe.on("load", function () {
                        loader.remove()
                    })
                } else {
            ...
            function setSize(width, height) {
                _alert.width(width), _alert.height(height)
            }

            function initFullScreen() {
                _modal.removeClass("_fullscreen"), _fullscreenToggler.removeClass("fa-compress"), _fullscreenToggler.removeClass("fa-expand"), _options.fullscreen ? (_modal.addClass("_fullscreen"), _fullscreenToggler.addClass("fa-compress")) : _fullscreenToggler.addClass("fa-expand"), _header.on("dblclick", function () {
                    ExternalMethods.toggleFullScreen()
                }), _options.disableFullscreen ? _fullscreenToggler.hide() : _fullscreenToggler.show()
            }
            ...
            function initResizable() {
                _alert.resizable({
                    ...
                    e.stopPropagation()
                }), _alert.resizable("option", "disabled", !_options.resizable), _options.resizable ? _fullscreenToggler.show() : _fullscreenToggler.hide()
            }

            function initReload() {
                _options.reload ? _reload.show() : _reload.hide()
            }
            ...
            function makeCenter() {
                ...
                top = 0 > top ? 0 : top, left = 0 > left ? 0 : left, _alert.css("left", left), _alert.css("top", top)
            }

            function setOptions(option, value) {
                switch (option) {
                    case "fullscreen":
                        _options.fullscreen = "boolean" == typeof value ? value : _options.fullscreen, _fullscreen = _options.fullscreen, initFullScreen();
                        break;
                    case "width":
                        _options.width = value || _options.width, setSize(_options.width, _options.height);
                        break;
                    case "height":
                        _options.height = value || _options.height, setSize(_options.width, _options.height);
                        break;
                    case "resizable":
                        _options.resizable = "boolean" == typeof value ? value : _options.resizable, initResizable();
                        break;
                    case "movable":
                        _options.movable = "boolean" == typeof value ? value : _options.movable, initMovable();
                        break;
                    case "reload":
                        _options.reload = "boolean" == typeof value ? value : _options.reload, initReload();
                        break;
                    case "disableFullscreen":
                        _options.disableFullscreen = "boolean" == typeof value ? value : _options.disableFullscreen, initFullScreen();
                        break;
                    case "type":
                        _options.type = value || _options.type, ExternalMethods.reload()
                }
            }

            function initEvents() {
                _fullscreenToggler.on("click", function (e) {
                    e.preventDefault(), e.stopPropagation(), ExternalMethods.toggleFullScreen()
                }).on("dblclick", function (e) {
                    e.stopPropagation(), e.preventDefault()
                }), _close.on("click", function (e) {
                    e.preventDefault(), e.stopPropagation(), ExternalMethods.close()
                }).on("dblclick", function (e) {
                    e.stopPropagation(), e.preventDefault()
                }), _reload.on("click", function (e) {
                    e.preventDefault(), e.stopPropagation(), ExternalMethods.reload()
                }).on("dblclick", function (e) {
                    e.stopPropagation(), e.preventDefault()
                }), $(window).on("resize", function () {
                    _modal.makeCenter && _modal.makeCenter()
                }), initMovable(), initFullScreen(), initResizable(), initReload(), makeCenter()
            }

            function initModalWindow() {
                loadContent(), _options.animate || _modal.removeClass("_animate"), _modal.appendTo("body"), setSize(_options.width, _options.height), window.glams.modal.insert(_modal)
            }

            function initExternalMethods() {
                for (var k in ExternalMethods) _modal[k] = ExternalMethods[k]
            }
            var _options = options || {};
            title = title || "", _options.width = _options.width || 500, _options.height = _options.height || 500, _options.fullscreen = "boolean" == typeof _options.fullscreen ? _options.fullscreen : !1, _options.disableFullscreen = "boolean" == typeof _options.disableFullscreen ? _options.disableFullscreen : !1, _options.resizable = "boolean" == typeof _options.resizable ? _options.resizable : !_options.disableFullscreen, _options.movable = "boolean" == typeof _options.movable ? _options.movable : !0, _options.reload = "boolean" == typeof _options.reload ? _options.reload : !0, _options.type = _options.type || "", _options.name = _options.name || "", _options.animate = "boolean" == typeof _options.animate ? _options.animate : !0;
            var _fullscreen = _options.fullscreen,
                _template = ['<div class="_modal-wrapper _animate">', '<div class="_modal">', '<div class="_modal-title">', '<span class="_title">' + title + "</span>", '<div class="_icons">', '<i class="fa fa-refresh _reload"></i>', '<i class="_toggle fa"></i>', '<i class="fa fa-times _close"></i>', "</div>", "</div>", '<div class="_modal-body"></div>', "</div>", "</div>"].join(""),
                _modal = $(_template),
                _alert = _modal.find("._modal"),
                _body = _modal.find("._modal-body"),
                _header = _modal.find("._modal-title"),
                _close = (_header.find("._title"), _header.find("._close")),
                _reload = _header.find("._reload"),
                _fullscreenToggler = _header.find("._toggle"),
                ExternalMethods = {
                    name: _options.name,
                    close: function () {
                        0 != _modal.triggerHandler("close") && (window.glams.modal.remove(_modal), _modal.remove())
                    },
                    reload: function () {
                        loadContent()
                    },
                    toggleFullScreen: function () {
                        !_options.disableFullscreen && _options.resizable && (_fullscreen = !_fullscreen, _fullscreenToggler.removeClass("fa-compress"), _fullscreenToggler.removeClass("fa-expand"), _fullscreen ? (_modal.addClass("_fullscreen"), _fullscreenToggler.addClass("fa-compress")) : (_modal.removeClass("_fullscreen"), _fullscreenToggler.addClass("fa-expand")))
                    },
                    setContent: function (_content) {
                        content = _content, ExternalMethods.reload()
                    },
                    makeCenter: function () {
                        makeCenter()
                    },
                    options: function (option, value) {
                        if ("string" == typeof option) setOptions(option, value);
                        else if ("object" == typeof option)
                            for (var o in option) setOptions(o, option[o])
                    }
                };
            return function () {
                initModalWindow(), initEvents(), initExternalMethods(), _modal.removeClass("_animate")
            }(), _modal
        }, $.aside = function (title, content, options) {
            var _options = options || {};
            title = title || "", _options.width = _options.width || 350, _options.height = _options.height || "100%", _options.fullscreen = !1, _options.disableFullscreen = !0, _options.resizable = !1, _options.movable = !1, _options.reload = "boolean" == typeof _options.reload ? _options.reload : !0, _options.type = _options.type || "", _options.name = _options.name || "", _options.animate = !1, _options.position = _options.position || "right";
            var _modal = $.modal(title, content, _options),
                _animationObj = {},
                _window = _modal.find("._modal");
            return _window.removeAttr("style"), _modal.addClass("_aside").addClass("_" + _options.position), _animationObj[_options.position] = 0, _window.animate(_animationObj, 500, "easeOutExpo"), delete _modal.makeCenter, delete _modal.options, delete _modal.toggleFullScreen, _modal
        }
    }(jQuery, window),
    function (d) {
        var l = 0;
        ...
        var f = {
            init: function () {
                return this.each(function () {
                    var a, g, c, f, h, k, m, e, b = d(this);
                    if (!b.data("isActive")) {
                        if (b.data("isActive", !0), this.plugin_count = l += 1, c = '<div class="icr__hidden" id="icr-container__' + this.plugin_count + '"></div>', k = b.prop("type"), f = b.prop("disabled"), h = b.prop("checked"), m = b.prop("name"), a = b.parent("label"), 0 < a.length) a.after(c), c = d("#icr-container__" + this.plugin_count), b.appendTo(c), a.appendTo(c), e = a.html(), a = e.replace(/<input["-=a-zA-Z\u0400-\u04FF\s\d\[\]\_]+>{1}/, ""), e = a.trim();
                        else {
                            if (a = b.prop("id"), a = d("label[for='" + a + "']"), a.after(c), c = d("#icr-container__" + this.plugin_count), b.appendTo(c), a.appendTo(c), !(0 < a.length)) throw Error("Label not found!");
                            e = a.html(), a = e.toString(), e = a.trim()
                        }
                        a = f ? h ? '<span class="icr disabled checked" id="icr-' + this.plugin_count + '">' : '<span class="icr disabled" id="icr-' + this.plugin_count + '">' : h ? '<span class="icr enabled checked" id="icr-' + this.plugin_count + '">' : '<span class="icr enabled" id="icr-' + this.plugin_count + '">', a = a + ('<span class="icr__' + k + '"></span>') + ('<span class="icr__text">' + e + "</span>"), a += "</span>", c.after(a), g = d("#icr-" + this.plugin_count),
                            function () {
                                g.on("click", function () {
                                    ...
                                    (h = b.prop("checked")) ? g.addClass("checked") : g.removeClass("checked")
                                })
                            }()
                    }
                })
    ...
    function ($, window, document, undefined) {
        $.notify = function (msg, options) {
            options = options || {};
            var container = $("._notification-container").length ? $("._notification-container") : $('<div class="_notification-container" />').appendTo("body"),
            ...
            return "object" == typeof msg ? notification.find("._msg").append(msg) : notification.find("._msg").html(msg), notification.prepend('<i class="fa fa-2x ' + icon + '"/> '), notification.addClass(type).appendTo(container), container.removeClass("top right bottom left").addClass(position), notification.fadeIn(), notification.close = function () {
                this.fadeOut(function () {
                    this.remove(), container.find("._notification").length || container.remove()
                })
            }, notification.on("click", function () {
                notification.close()
            }), delay > 0 && setTimeout(function () {
                notification.close()
            }, 1e3 * delay), notification
        }
